        ...
        for name in tqdm(angio_list):
            try:
                for image in os.listdir(self.data_path + "/" + name + "/SR 1"):
                    if image[0] == 'I':
                        i = 0
                        for img in self.vid_to_array(self.data_path + "/" + name + "/SR 1" + "/" + image):
                            i = i + 1
            ...
            for name in os.listdir(self.data_path + '/' + cd):
                if name[0] == 'd':
                    for image in os.listdir(self.data_path + "/" + cd + '/' + name):
                        try:
                            if image[0] == 'S':
                                nib.save(nib.Nifti1Image(self.vid_to_array(self.data_path + "/" + cd + '/' + name + "/" + image),
                                                         self.affine),
